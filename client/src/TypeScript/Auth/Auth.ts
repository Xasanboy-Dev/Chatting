...
export async function postDataUser(name: string, surname: string, email: string, password: string) {
    try {
        const result = await axios.post(`http://localhost:8080/user/register`, { email, surname, name, password })
