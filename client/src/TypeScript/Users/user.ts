...
export async function getArchivedUsers(userID: number, token: string) {
  return await axios.get(`http://localhost:8080/user/archivedUsers`, {
    headers: { Authorization: token, Accept: userID },
  });
}
...
export async function deleteAchivedUser(token: string, chattingUserId: number) {
  const result = await axios.delete(`http://localhost:8080/user/archieve`, {
    headers: { Authorization: token, Accept: chattingUserId },
  });
  console.log(result.data);
}

export async function getChattingUserById(userID: number, chattingID: number) {
  return await axios.put(`http://localhost:8080/user/chattingUserID`, {
    userID,
    chattingID,
  });
}
