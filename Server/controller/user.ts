...
export async function getChattingUserID(req: Request, res: Response) {
  try {
    const { userID, chattingID } = req.body;
    if (userID && chattingID) {
      const user = await checkUserExistByID(+userID);
      const chatting = await checkUserExistByID(+chattingID);
      if (user && chatting) {
        return res
          .status(200)
          .json({ message: "CHattingUser", user: chatting });
      } else {
        return res
          .status(500)
